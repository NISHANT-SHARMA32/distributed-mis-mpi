...
enum MessageTag1
{
	share_Prob,				  // [0,1]
	mark_Status,			  // {0,1}
	mis_Or_ComMis_Or_Continue //	{0,1,2}
};


void Logg(int id, string message, int iteration=0)
{
	ofstream outFile;
	outFile.open(to_string(id) + ".txt" + to_string(id), ios_base::app);
	int k =1;
	while(k<2){
...
// Ghaffari Beeping Algorithm


// In this method, the node sends its current round's probability to all of its neighbors
double yshh_rnd1(int id, int provb_v, vector<int> *vecNeiId)
{
	Logg(id, "...Round yshi 1 Start...");

	double randNum = provb_v; // my current probability 

	Logg(id, " randNum yshi: " + to_string(provb_v));

	for (int i = 0; i < vecNeiId->size(); i++)
		MPI_Send(&randNum, 1, MPI_DOUBLE, vecNeiId->at(i), MessageTag1::share_Prob, MPI_COMM_WORLD);

	Logg(id, "...Round 1 Finish...");

	return randNum;
}

// In this method, the node receives the probabilities of the neighbors and calculates the effective_degree
double yshh_rnd2(int id, vector<int> *vecNeiId)
{
	Logg(id, "...Round 2 Start...");

	double *arrRecRandNum = new double[vecNeiId->size()]; // an array that will contain the probabilities of the neighbors
	double effective_degree = 0;
	for (int i = 0; i < vecNeiId->size(); i++)
	{
		double recRandNum;
		MPI_Recv(&recRandNum, 1, MPI_DOUBLE, vecNeiId->at(i), MessageTag1::share_Prob, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		arrRecRandNum[i] = recRandNum;
		effective_degree += recRandNum;
	}

	delete[] arrRecRandNum;

	Logg(id, " effective_degree: " + to_string(effective_degree));

	Logg(id, "...Round 2 Finish...");
	return effective_degree;
}

// In this method, mark yourself with this->prob_v probability, and send the mark status of the node to neighbours
void yshh_rnd3(int id, vector<int> *vecNeiId, double effective_degree, int provb_v)
{
	Logg(id, "...Round 3 Start...");
	...
	// {
	// 	this->prob_v = this->prob_v / 2;
	// }
	// else
	// {
	// 	this->prob_v = min(2 * this->prob_v, 0.5);
	// }

	// Define the Bernoulli distribution with p = 0.5 (50% chance of success)
	std::bernoulli_distribution distribution(provb_v);

	// Generate random numbers from the Bernoulli distribution
	bool result = distribution(gen);
	Logg(id, " marking result : " + to_string(result));

	for (int i = 0; i < vecNeiId->size(); i++)
		MPI_Send(&result, 1, MPI_INT, vecNeiId->at(i), MessageTag1::mark_Status, MPI_COMM_WORLD);

	Logg(id, "...Round 3 Finish...");
}

// In this method, each node receives the sent messages that containes the marked status of its neighbors.
// If there is no neighbor with marked status, join MIS.
void yshh_rnd4(int id, vector<int> *vecNeiId, double effective_degree, double provb_v)
{
	Logg(id, "...Round 4 Start...");
	if (effective_degree >= 2)
	{
		provb_v = provb_v / 2;
	}
	...
	{
		provb_v = min(2 * provb_v, 0.5);
	}

	bool *arrRecNodeStatus = new bool[vecNeiId->size()];
	bool flag = 0;
	for (int i = 0; i < vecNeiId->size(); i++)
	{
		bool recNodeStatus;
		MPI_Recv(&recNodeStatus, 1, MPI_INT, vecNeiId->at(i), MessageTag1::mark_Status, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	...
	if (flag == 0)
	{
		Status NodeStatus = Status::Mis;
	}

	delete[] arrRecNodeStatus;

	Logg(id, " NodeStatus: " + to_string(NodeStatus));

	Logg(id, "...Round 4 Finish...");
}

/*In this method, if the status of a node is MIS, then it send "remove yourself" message to its neighbors.
in the explanation of the next method.*/
void yshh_rnd5(int id, vector<int> *vecNeiId)
{
	Logg(id, "...Round 5 Start...");
	...
	if (NodeStatus == Status::Mis)
	{
		int msg = 0;
		for (int i = 0; i < vecNeiId->size(); i++)
			MPI_Send(&msg, 1, MPI_INT, vecNeiId->at(i), MessageTag1::mis_Or_ComMis_Or_Continue, MPI_COMM_WORLD);
	}
	else if (NodeStatus == Status::ComMis) // most probably will not run here but in round 6
	{
		int msg = 1;
		for (int i = 0; i < vecNeiId->size(); i++)
			MPI_Send(&msg, 1, MPI_INT, vecNeiId->at(i), MessageTag1::mis_Or_ComMis_Or_Continue, MPI_COMM_WORLD);
	...
	Logg(id, "...Round 5 Finish...");
}

/*In this method, you receive any MIS message, turn to comMis, and resend your status to neighbours. 
*/
void yshh_rnd6(int id, vector<int> *vecNeiId)
{
	Logg(id, "...Round 6 Start...");
	Status NodeStatus;
	if (NodeStatus == Status::Unknown)
	{
		...
		{
			int recId;
			MPI_Recv(&recId, 1, MPI_INT, vecNeiId->at(i), MessageTag1::mis_Or_ComMis_Or_Continue, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			if (recId == 0 && vecNeiId->at(i) < id)
			{
		...
		else if (NodeStatus == Status::ComMis) 
		{
			int msg = 1;
			for (int i = 0; i < vecNeiId->size(); i++)
				MPI_Send(&msg, 1, MPI_INT, vecNeiId->at(i), MessageTag1::mis_Or_ComMis_Or_Continue, MPI_COMM_WORLD);
		...
		Logg(id, "...Round 6 Finish...");
	}
}

/*Fo receiving message from nodes which turned comMIS in round 6.
...
The update task is necessary because if it is not done, in the next iteration, the node waits for receiveing messages from the deactive neighbors.*/
void yshh_rnd7(int id, vector<int> *vecNeiId)
{
	Logg(id, "...Round 7 Start...");
	int flag = 0; // did this node turn into comMis in this function itself
	Status NodeStatus;
	if (NodeStatus == Status::Unknown)
	{
		vector<int> vecComMisId;
...
double MisAlgorithm::ysh_rnd2(int id, vector<int>* vecNeiId, int a)
{
	...
	delete[] arrRecRandNum;
	mis_debugger(1);
	Logg(id, " maxRecRandNum: " + to_string(maxRecRandNum), 1 );

	Logg(id, "...Round 2 Finish...", 1 );
...
while (i < vecNeiId->size()) {
	...
	Logg(id, " NodeStatus: " + to_string(NodeStatus), 1 );

	Logg(id, "...Round 4 Finish...", 1 );
}
